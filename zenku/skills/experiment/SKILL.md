...
Start from a note in the project's ideas folder. If there is none, run `zenku:capture-idea` first: an experiment with no idea above it has nothing to be measured against and nothing to be promoted later.
...
Unknowns that turn up sideways go to the idea, or to `zenku:capture-idea` if they belong to a different objective, or to `zenku:capture-backlog` if they are just work. They do not get absorbed into this experiment's scope.
