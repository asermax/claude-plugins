...
**Under an adventure, do not.** Its lore gets written once, at the end, out of every quest's design at once. A slice that documents itself as it lands produces one page per slice, each describing a part of a shape nobody has seen whole yet, and reconciling those afterwards costs more than writing the page once from the quests. Leave the detail on the quest and go on.
