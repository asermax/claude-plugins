...
# Ensures agent.py and browser-daemon.py always run in background
...
if [[ "$tool_name" == "Bash" ]]; then
    ...
    # Check if command is calling agent.py or browser-daemon.py
    if [[ "$command" == *"/agent.py"* || "$command" == *"/browser-daemon.py"* ]]; then
