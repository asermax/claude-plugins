...
- **Turns out to carry a real unknown** — the fix depends on something nobody knows, or the "obvious" approach has a question in it? **Stop.** Leave the item open, say what the unknown is, and offer `zenku:capture-experiment` so it gets an objective and an experiment. Do not grow the item into a research project; that is the failure this split exists to prevent.
