...
- **Curly braces and `<br/>` in sequence messages break some renderers.** A sequenceDiagram message carrying `{id}`-style braces or a `<br/>` tag parses cleanly, so the validator passes it, while some renderers (Obsidian's, observed) fail at the first such message and report the parse error on a later line. Keep sequence messages plain text; put the content in a note when it must be split. `<br/>` inside flowchart node labels and sequence notes renders fine, and commas, slashes and parentheses in messages render fine.
