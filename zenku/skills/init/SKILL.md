...
Then the two sentences that matter: **everything under the vault is the project's now**; edit any template, add a folder, rewrite the charter, and the skills will follow it. And what to do next: capture an objective with `zenku:capture-idea`, write down something already decided with `zenku:capture-backlog`.
