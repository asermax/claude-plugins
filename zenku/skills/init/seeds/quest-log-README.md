...
**Both record types use the same field, and the tag says which vocabulary applies.** An adventure's `kind` is `venture` or `raid`; a quest's says how it gets solved:
...
**One effort owns one spike worktree**, on a branch that is never merged, and every spike works in there. The venture's first spike creates it. Spikes compound, and a fresh checkout each time orphans whatever the last one fetched or cached, so the second question ends up paying for the first one twice.
...
**The raid strikes it**, after the durable note is written and never before. A venture nobody raids strikes its own when it closes. Either way, anything that mattered has to be in the note or in the solved quests by then, because that is the last moment it is recoverable.
