...
The charter owns the shape. The framework asserts four things about the content, and only these four:
...
**Explain the code, do not reprint it.** This is the other half of mechanism-first: a note explains, and explanation is not transcription. A declaration, a constructor body or a whole function belongs in the file it lives in, where it is typechecked and cannot fall out of date. What a reader cannot get from that file is why each part exists and who asks it, so the surface goes in a table of what each member answers and the behaviour goes in a diagram or in prose. A snippet earns its place only where the code *is* the insight (an exact formula, a guard whose precise form is the point), and then it is two to four lines rather than a block. Written the other way a note comes out a third fenced code, and the copy rots.
...
**Use a diagram whenever the subject has a shape.** A state machine, a data model, a sequence, a tree or a request's path through the system all read faster as a diagram than as a paragraph describing one. Follow the charter's diagram convention; where it names mermaid, do not draw it in ASCII.
