...
- **The `.mahou/` local layer.** `.mahou/basics.md` for standing rules and for naming the docs folder, `.mahou/<skill>.md` per skill, `.mahou/wiki/` for local entries. Every skill's first lines are "load mahou:basics, then read `.mahou/<skill>.md` if present". `basics` in turn loads `mahou:wiki` and reads its root indexes, so every skill knows which topics have entries; the entries themselves are read only where a skill says so (the two review skills) or when the user asks for a conventions check, never brought unasked into a question. Artifacts a skill produces for the user and keeps between rounds go to `.scratch/<change-slug>/` at the project's root, gitignored; throwaways stay in `/tmp`.

**Skills:**

*Process, in the order work moves:* `explore` (user-steered investigation, one question at a time, facts fetched through `lookup` so the reading stays out of context, writes nothing) → `shape` (settle behavior-level decisions as a design tree; opens by placing the decisions the user brings as ✓ leaves under pending areas, then works one area at a time) → `design` (system design one area at a time, type references under `references/types/` for data model, schema (a structured value a schema validates rather than a table: JSON column contents, cross-system payloads, tool arguments), data flow, rules, lifecycle, component architecture, contract and UI, hands the tree to `write-documentation`; the branch list is shown as the tree itself, the first branch opens with its approval and each later one with the approval of the previous branch's rendering; a branch opens by stating what is already settled as ✓ leaves and waiting for corrections before the frontier is asked; the reference's representation is rendered directly, asked only when no reference matches; sub-branches describing one thing may render as one representation once settled, and a behaviour another note or branch already settles is referenced in one sentence rather than redrawn) → `program-architecture` (turns a settled design into the architecture of the program that implements it, as a design tree of package layout, control flow, then a contract per part, extending `mahou:design` and reusing its types; below the system, above function signatures, file lists and tests; a part's existing note is extended rather than duplicated) → `spike` (answer a design question that reading and asking cannot settle, by building the smallest working version of the real thing and running it against reality; throwaway code that never merges, one question per spike, rounds under a strategy from `references/strategies/`, code and findings in the change's scratch folder at `spikes/<question-slug>/`) → `test-design` (Classification Tree Method on the settled design, before any code: one tree per repository, entry points or flows as roots, classifications, partitions, then a combination matrix per branch with rows marked `test`, `manual` or both; candidates derive only from settled leaves and facts read from the code, the user settles every level; test level and naming come from the wiki's testing entries and the repository's own conventions, whether a surface has a runner is a fact read from it; settled trees and matrices written to the scratch folder at `test-design/<repo>.md` so implement reads them across sessions; ported from `filadd-experimental-dev` 2026-09, the Filadd stack ordering replaced by "from the repository others depend on outward") → `implement` (one subagent per repository with a full brief: design notes and summary, tasks, its matrix rows verbatim (read from the scratch folder at `test-design/<repo>.md` when test-design wrote them there), repository rules, the `report` shape; relays results and flags as decisions, then a subagent runs `agentic-review` per repository as it lands and `validate` one repository at a time; task tracker generalized, the tracker, its tool and its states come from `.mahou/implement.md` or the user, and the brief names them concretely; ported 2026-09 with the Notion Board states and the `local-services` load removed) or code written by hand or another tool → `validate` (the manual rows from `test-design`, or user-named scenarios, driven through a browser via `agent-browser` or over HTTP via `curl`, references under `references/`; how the system runs comes from `.mahou/validate.md` or the project's own setup; capture-mutate-revert discipline for local data, an HTML report artifact in the change's scratch folder at `reports/` built as rows run, **fixes nothing**, every failure a decision; renamed from filadd-dev's `validate-feature` 2026-09 with the rubric dropped because `test-design` already covers it, the three Filadd surface references collapsed to two generic ones, and the "fix and re-run until clean" rule inverted) → `code-blast-radius` (one `usage-tracer` per changed element, reports only where behavior changes for someone) → `agentic-review` (typed or invoked by `implement`, `disable-model-invocation` dropped 2026-09 for that; five facet reviewers per repo, fixes within the change's scope, up to three rounds; **the one skill that edits without asking**, bounded to changeset files, uncommitted, every fix and skip reported) → `human-review` (typed only; `plannotator` annotations verified and settled before any edit) → `write-documentation` (overview, then the template's sections the subject needs, `superpowers:unslop` pass, a `documentation-reviewer` pass against the charter, shown before written; a stripping rule set — name roles not products, link dependencies, no library names, no current-implementation description, no ownership, no prototype controls, no comparison with today — keeps session artifacts out of the shared note) → `commit-changes` (typed only; conventional commits, grouping confirmed first, never pushes, no co-author trailers rule) → `release-check` (what breaks when the change's repositories are released in a given order, and the order that breaks nothing; boundaries come from the diff plus the user since mahou has no dependency map to read, one `compat-checker` per boundary and `code-blast-radius` alongside, findings verified against their cited lines before reporting; ported 2026-09 with project-knowledge's dependency script, `prepare-repo` and the filadd org replaced by diff-derived boundaries, inline repo prep and the remote's owner) → `land` (from branches to merged pull requests in the safe order: runs `release-check`, opens every PR through `create-pull-request`, babysits them in parallel with one poll loop each — a review is reported verbatim and waits for the user, `rebase` when behind, stop on a failed check, record on merge — then walks the confirmed order, watching each merge's CI run and stopping for the user's production confirmation before the next, the release following each project's own process; never merges a PR itself; ported 2026-09 with `address-pr-review` left out by request and `release-repo` dropped because generic projects release from their own CI).

*Entry points and tooling:* `guide` (matches a goal against skill descriptions dynamically, no routing table; hands off without summarizing or chaining), `init` (typed only; seeds the docs charter, per-folder READMEs with a hand-kept index table, `_templates/note.md` carrying the overview-plus-menu shape adapted from filadd-docs's section menu minus Issue, Possible Solutions and Edge Cases, emoji heading prefixes kept, and a `## Docs` CLAUDE.md block; never overwrites, re-run to add a folder; `init/seeds/` is the only place a template body lives), `wiki`, `learn` (typed only; discovery mode collects the session's friction points and gives a take on each, directed mode works the ask; drafts every item in one reply, each under its heading, written to the scratch folder and passed through `superpowers:unslop` before being shown, approved per item; fidelity rule: reproduce the session's patterns exactly). Asked for by name: `create-pull-request` (opens one PR, base found by commit distance, title and body from every commit on the branch, waits for confirmation before creating), `rebase` (brings a branch up to date with its base, verifies against the suite, never pushes; ported 2026-09 for `land`).

*Loaded, never typed:* `basics`, `docs`, `design-tree` (frontier rounds, numbered questions across the session, questions written straight into the reply and never through a tool, a section on what a question carries: the context the user needs (earlier answers that bear on it, what the answer changes) and never the ways to answer it, with what counts as a proposed answer and a reread before every round, settled decisions stated as ✓ leaves and never re-asked, a Scope section that gates hypotheticals (a missing value, a failing step, a later feature) behind one "do we consider X?" question per branch with deferred ones recorded where the loading skill says, the whole tree shown when a branch opens or settles and the worked branch alone between rounds, facts only on request), `changeset` (repos by absolute path and `git -C`, never `cd`; base from `@{u}`, then `origin/HEAD`, then `git remote show origin`, then ask, never guess between `main` and `master`; three-dot diff; introduced vs pre-existing), `prototype` (one self-contained HTML file in the change's scratch folder at `prototypes/`, reopened in place each round; a control strip switches states, alternatives lettered on one page in the first round, corrections applied as given afterwards, reconciled against the settled leaves before a UI branch renders; loaded by `explore` when a screen question comes up and by the `ui` design type, brought over from `filadd-experimental-dev` 2026-09 with no Filadd dependency to strip), `lookup` (where a fact comes from: routing between `code-scout` per repository, `docs-scout` against the project's docs folder, `researcher` for anything outside the project, or a direct trial; verified vs asserted kept apart in the reporting; loaded by `explore` and `design-tree`, ported 2026-09), `report` (the shape of what a skill hands the user when the work is done: findings, an investigation's answer, a change summary; loads the unslop rules and holds them while writing rather than running the pass, leads with the answer, diagrams what reads clearer as a picture, separates the verified from the claimed, cuts what does not bear on the ask, structures by reader need rather than by source; ported from `filadd-experimental-dev` 2026-10 with the unslop and mermaid loads re-pointed at superpowers).

**Agents** (all write findings and edit nothing, `model: sonnet`): the four generic facet reviewers plus `conventions-reviewer` (loads `mahou:wiki` for the technologies in the diff, cites the entry, flags stale `verified` dates); `usage-tracer`; `code-scout` and `docs-scout` (one question, one repo or the docs folder, report under forty lines); `compat-checker` (one service boundary, new side against deployed other side in both directions, migrations against instances on the old code, a client still running an old build; dispatched by `release-check`, ported near-verbatim 2026-09); `documentation-reviewer` (a draft against the charter, folder README and template, judged the way `docs` says the template is used; a menu section left out is never a finding).

**Cross-plugin dependencies:** `superpowers:unslop` (the writing pass in `write-documentation`, the rules held while writing in `report`) and `superpowers:mermaid-validation` (diagrams in `design`, `write-documentation` and `report`). Both degrade to "say so and continue" when absent. External CLIs: `plannotator` (`human-review`) and `agent-browser` (`validate`'s browser reference loads its skill content via `agent-browser skills get core --full`).

**Deliberately absent:** `address-pr-review` (dropped by request; `land` reports review findings verbatim and waits for the user instead of delegating them), `scope-tasks` and `update-pitch-context` (Notion Board and Recorder Context writers with nothing in mahou to map onto), `standards-reviewer` as written (replaced by the wiki-reading conventions reviewer), the Filadd scouts, `create-doc-changelog`, the `design-doc` command, filadd-docs's emoji section menu and phrase lists, and any document-type taxonomy: those are structure, and structure is the project's.
...
- **unslop** (in superpowers): Four locally added patterns at the end of the `### Plain speech` section, not present upstream — preserve them on sync (manual merge, never a straight copy). **34. Describing the message instead of writing it** covers spans that refer to the text rather than the subject ("Two things to take into account:", "I should mention that...", "which brings us to the next point") at any position in a sentence, plus counting headings, which upstream does not cover at all. **35. Restating what you just said** covers a second pass over an idea at the same level of detail; upstream's 16 covers only the narrow case of a bold lead restating its own line. **36. Pronoun referent overload** covers the same pronoun standing for two different things in one sentence, or a dropped relative pronoun whose wrong attachment is the easier parse; the fix names the referent or restores the "that". Upstream's 28 has the backtrack test that finds these sentences, but its split-the-sentence fix cannot repair them. **37. Negation where a positive fits** covers double negatives ("not uncommon", "nothing prevents") and "not about" framing that say what something is not while leaving what it is unstated; the fix states the positive fact, and a single honest negative stays. All four mirror the user's global CLAUDE.md `# Writing style` section, so they should move together. An earlier local pattern, mannered prose, was dropped in the 2026-09 sync because upstream grew its own **32. Mannered prose** with the same intent (upstream's 33 is **Over-compression**, unrelated). The frontmatter is local as well: upstream's `disable-model-invocation: true` is dropped and the `Must always apply.` description is replaced by one scoped to document writing, because mahou's `write-documentation` invokes the skill by name and cannot if the flag is set. Keep the flag out and the scope in on sync. The whole method is local too, replacing upstream's `## Process`: a two-round pass where the first round is one opus agent with every rule scanning the pass documents, the main agent verifies and applies, and the second round is a fresh agent holding the first round's applied report, flagging only findings whose fix is gone or undone. `## How to scan` carries the fresh-agent rationale, `### Prompts` carries both templates (the scan prompt and the verification prompt), and `## Between rounds` carries the claim-verification steps for both rounds. A renumbered copy of the rules lives in the Filadd marketplace's `filadd-docs:unslop` skill (`plugins/docs/skills/unslop/SKILL.md`), and the two rule sets move together. A copy of the method lives in `filadd-docs:doc-writing`'s `## Unslop Pass` (`plugins/docs/skills/doc-writing/SKILL.md`); it repeats full-scan rounds until one finds nothing, up to five, while this copy runs two rounds and makes the second a verification of the first. Everything else is upstream's
...
- Upstream repositories:
  ...
  - `~/workspace/random/cursor-plugins` - `unslop` from `pstack/skills/` (synced into **superpowers**, plus four locally appended patterns). `cursor/plugins` is a monorepo and `pstack` alone ships ~40 skills (the `principle-*` family, `tdd`, `architect`, `why`, `swarm`); none of the rest are tracked, add one only on explicit request
