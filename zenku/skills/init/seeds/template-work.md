...
**Delete this whole section, heading included, when creating a work item.** `zenku:work-on-backlog` adds the heading back when the work is done. An `open` item carries no `## Resolution` at all.

What was actually done, where it landed, and which notes moved.
