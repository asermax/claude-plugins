...
## §1 How a skill resolves the project
...
**Never hardcode a trunk name.** Resolve it, and note that the first command fails outright in a very common case, so treat this as an ordered chain rather than one command with a vague fallback:

```bash
git symbolic-ref --short refs/remotes/origin/HEAD   # → "origin/main"; strip the "origin/" prefix
```

That errors with `fatal: ref refs/remotes/origin/HEAD is not a symbolic ref` whenever the local ref was never seeded, which is the default state of any clone nobody ran `git remote set-head` in. It is not a sign of anything wrong. When it fails, in order:

1. `git remote show origin` and read its `HEAD branch:` line. Accurate, but it contacts the remote, so it is slow and fails offline.
2. If there is no remote, or it is unreachable: list local branches. Exactly one means that is the trunk.
3. Otherwise **ask.** Do not guess between `main` and `master` by checking which exists, because a repository can carry both.

Once resolved, **offer to run `git remote set-head origin -a`**, which seeds the ref permanently so the first command works from then on. One command, and it fixes the cause rather than the symptom.
...
## §4 What the framework never bends on
...
| `zenku:capture-idea` | Write an objective and its unknowns into the lab |
