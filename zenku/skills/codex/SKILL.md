...
An **adventure** is too big and too foggy for one session. It carries a **destination** — what reaching the end looks like — plus its trials and an index of the quests already **solved**. The destination settles first, because it is what makes something **out of scope** rather than a trial.
...
**Find the path.** While an adventure still has trials, the work produces decisions, not deliverables. The pull to just build is usually the signal you have reached the edge of what is known.

**Keep the pace.** One quest per session, research excepted. A session that solves three quests has usually guessed at two of them.
...
- The quest log's **status vocabulary**, the four **quest types** the routing runs on, and the **tag** that says whether a record is an adventure or a quest. These are process rather than structure: the dispatcher branches on them, so a project renaming one breaks the routing silently. A project that wants different words changes the framework, not just its vault.
...
- A **quest's**: what it settles, its design, its answer — plus the four fields the routing runs on (`status`, `type`, `adventure`, `blocked_by`).
...
| The branch | `**Branch**` | One branch per adventure. Spikes are separate and never a choice: one worktree per adventure, on a branch that is never merged. See `zenku:spike`. |
