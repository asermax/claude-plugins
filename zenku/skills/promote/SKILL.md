...
git rev-parse HEAD                                  # the spike tip; keep it for the check below
git for-each-ref --contains HEAD --format='%(refname)'   # what else reaches these commits
git reset --hard <trunk>                            # every spike commit comes off in one move
git for-each-ref --contains <spike sha> --format='%(refname)'   # and what still does
```
...
**Run `for-each-ref` before and after, and read both.** The reset moves one branch, and any other ref that reaches those commits keeps the whole spike alive: a tag someone made, a second branch, or a pushed copy on the remote. A spike that survives on `origin` defeats the point of discarding it, so if the branch was pushed, deleting the remote copy is part of this step and not a follow-up. Say what still reaches the commits after the reset, and deal with each.

**Do not revert on top instead.** A revert commit leaves the spike one commit back on a branch that then merges into the trunk, which puts every shortcut into the project's permanent history: reachable, greppable, and offering itself to the next person who wonders how the old version did it. Dropping the commits is what keeps them out.

The dropped commits stay reachable through the reflog. If the user wants a durable reference (a spike whose graduation has not happened yet, something worth looking at again), **offer to tag the branch tip before resetting**, and say you did. A tag is the deliberate version of what `for-each-ref` is checking for.
