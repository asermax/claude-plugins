...
**Cross-plugin dependencies:** `superpowers:unslop` (the writing pass in `write-documentation`, the rules held while writing in `report`) and `superpowers:mermaid-validation` (diagrams in `design`, `write-documentation` and `report`). Both degrade to "say so and continue" when absent. External CLI: `plannotator`.
