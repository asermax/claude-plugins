...
**Is there behaviour with no coverage?** New or changed behaviour without a new or updated test. Name the specific gap rather than a count: the case that would break silently.
...
The project owns its conventions. You are checking consistency *with this project*, never conformance to a general standard. If the codebase does something unusual on purpose, that is not a finding. If you cannot tell whether it is on purpose, say that rather than assuming it is not.
