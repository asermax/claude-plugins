...
2. Look at every skill whose name starts with `mahou:` and match the goal against their descriptions. Skip the ones that are loaded by other skills rather than run (`basics`, `docs`, `design-tree`, `changeset`, `prototype`) and the ones marked typed-only; neither is a goal to route toward.
