...
- **The tests.** The matrix rows of its repository, verbatim, with each row's expected outcome and kind. The `test` rows are the tests it writes, at the level the repository's conventions name and named after what they prove. The `manual` rows are for later; it does not run them.
